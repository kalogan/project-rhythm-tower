...
export interface PressResult {
  ...
  /** The cue this press resolved, or null if the press was a stray (ignored). */
...
export function pressButton(session: FloorSession, button: Button, timeSec: number): PressResult {
  ...
  let verdict: Verdict;
  if (cue.kind === 'decoy') {
    verdict = 'wrong'; // you were supposed to ignore it
  } else {
    const expected = expectedButtons(cue, session.mapping);
    verdict = expected.includes(button) ? timingVerdict(bestErr, session.windows) : 'wrong';
  }
 ...
 * becomes 'avoided' (success); anything else unresolved becomes 'miss'. Returns the
